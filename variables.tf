...
variable "webhook_lambda_zip" {
  ...
  type        = string
  default     = "modules/download-lambda/webhook.zip"
}
...
variable "runners_lambda_zip" {
  ...
  type        = string
  default     = "modules/download-lambda/runners.zip"
}
...
variable "runner_binaries_syncer_lambda_zip" {
  ...
  type        = string
  default     = "modules/download-lambda/runner-binaries-syncer.zip"
}
...
variable "runner_run_as" {
  ...
  type        = string
  default     = "ubuntu" # amazon linux(default)で実行する場合にはec2-userを指定
}
...
variable "enable_userdata" {
  ...
  type        = bool
  default     = false # カスタムAMIを利用するため不要
}
...
variable "ami_filter" {
  ...
  default = {
    state = ["available"]
    name  = ["ubuntu-custom-java-node-php*"]
  }
  validation {
